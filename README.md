...
The $\Psi$ operator from the paper (when applied with the group $G$ generated by rotations and translations) corresponds to the composition of the custom PyTorch layer `glp_rotation_stack` followed by a 3D convolution with a $n \times n \times 1$ kernel.
The $\Xi$ operator (again, with a group $G$ generated by rotations and translations) corresponds to the `glp_rotation_pool` custom layer.
